...
## 3. Loading and Filtering the CasosMI Database

The CasosMI database from Descontamina Colombia records antipersonnel mine incidents. We filter by year (1994–2024) and weapon type (mines only).

```{python}
#| label: load-casosmi

df_mi = pd.read_excel("../data/CasosMI_202509.xlsx", sheet_name=0)
print(f"Total records: {len(df_mi):,}")

df_mi = df_mi[(df_mi["Año"] >= 1994) & (df_mi["Año"] <= 2024)]
print(f"Records 1994-2024: {len(df_mi):,}")

mask_arma = df_mi["Tipo de Armas"].str.contains(
    r"MINAS",
    case=False, na=False, regex=True
)
df_mi = df_mi[mask_arma].copy()
print(f"Records with mines: {len(df_mi):,}")

print("\nBreakdown by weapon type:")
for arma, cnt in df_mi["Tipo de Armas"].value_counts().items():
    print(f"  {arma}: {cnt}")

df_mi["es_desminado"] = df_mi["Tipo de Evento"].str.contains(
    "DESMINADO MILITAR", case=False, na=False
)

n_desminado = df_mi["es_desminado"].sum()
n_otros = len(df_mi) - n_desminado
print(f"\nEvent classification:")
print(f"  Military demining: {n_desminado}")
print(f"  Other events:     {n_otros}")
```

### Map: Antipersonnel Mine Events

Each point represents a mine event. Green points indicate military demining operations; red points indicate all other mine-related events.

```{python}
#| label: fig-mines-map
#| fig-cap: "Antipersonnel Mines by Municipality — Colombia 1994–2024 (CasosMI)"
#| fig-width: 12
#| fig-height: 16
...
df_otros = df_mi[~df_mi["es_desminado"]]
df_desmin = df_mi[df_mi["es_desminado"]]

ax2.scatter(
    df_otros["Longitud"], df_otros["Latitud"],
    s=5, color="#cc0000", alpha=0.4, zorder=2, label=f"Mines / Explosive devices ({n_otros:,})"
)

ax2.scatter(
    df_desmin["Longitud"], df_desmin["Latitud"],
    s=12, color="#22aa22", alpha=0.7, zorder=3,
    edgecolors="#115511", linewidths=0.3,
    label=f"Military demining ({n_desminado:,})"
)
...
fig2.text(
    0.5, 0.02,
    f"Total events: {len(df_mi):,} | Source: Descontamina Colombia – CasosMI",
    ha="center", fontsize=9, color="#666666"
)
...
victimas_col = "Total de Víctimas del Caso"
df_mi_plot = df_mi.copy()
df_mi_plot["victimas"] = df_mi_plot[victimas_col].fillna(0).astype(int)
...
fig_v.text(
    ...
    f"Source: Descontamina Colombia – CasosMI",
    ha="center", fontsize=9, color="#666666"
)
...
We combine both datasets at the municipality level and classify each municipality into one of four quadrants based on the median values of conflict events and mine events.

```{python}
#| label: bivariate-data

mi_por_muni = (
    df_mi.groupby("Código DANE de Municipio")
    .agg(
        minas=("ID Caso", "count"),
        municipio_mi=("Municipio", "first"),
        depto_mi=("Departamento", "first"),
        lat_mi=("Latitud", "mean"),
        lon_mi=("Longitud", "mean"),
    )
    .reset_index()
)

conflict_por_muni = (
    df_col.groupby(["departamento", "ciudad"])
    .agg(conflicto=("id", "count"))
    .reset_index()
)
...
dane_lookup = {}
for _, row in mi_por_muni.iterrows():
    dane_lookup[(normalize(row["depto_mi"]), normalize(row["municipio_mi"]))] = row["minas"]
...
for idx, row in gdf_analysis.iterrows():
    k = (row["key_dept"], row["key"])
    if k in dane_lookup:
        gdf_analysis.at[idx, "minas"] = dane_lookup[k]
        continue
    for (dk, mk), count in dane_lookup.items():
        if mk == row["key"] or (dk in row["key_dept"] and mk in row["key"]):
            gdf_analysis.at[idx, "minas"] = count
            break
...
for _, row_mi in mi_por_muni.iterrows():
    mk = normalize(row_mi["municipio_mi"])
    if mk in varname_lookup:
        g_idx = varname_lookup[mk]
        if gdf_analysis.at[g_idx, "minas"] == 0:
            gdf_analysis.at[g_idx, "minas"] = row_mi["minas"]

total_minas_mapped = gdf_analysis["minas"].sum()
print(f"Mines mapped to municipalities: {total_minas_mapped:,} of {len(df_mi):,}")
...
ax3.set_ylabel("← Few mines          Mine events (CasosMI)          Many mines →",
               fontsize=12, labelpad=10, color="#333333")
ax3.set_title(
...
fig4.text(
    0.5, 0.02,
    "Sources: UCDP GED v25.1 + Descontamina Colombia (CasosMI) | "
...
df_otros_q = df_mi[~df_mi["es_desminado"]]
df_desmin_q = df_mi[df_mi["es_desminado"]]
...
ax5q.scatter(
    ...
    edgecolors="#115511", linewidths=0.3,
    label=f"Military demining ({len(df_desmin_q):,})"
)
...
minas_anual = df_mi.groupby("Año")["ID Caso"].count().reindex(years, fill_value=0)
...
ax_m.plot(
    ...
    zorder=4, label="Antipersonnel mine events (CasosMI)"
)
...
desminado_anual = df_mi[df_mi["es_desminado_all"]].groupby("Año")["ID Caso"].count().reindex(years, fill_value=0)
incidentes_anual = df_mi[~df_mi["es_desminado_all"]].groupby("Año")["ID Caso"].count().reindex(years, fill_value=0)
...
- **Mine Events**: Descontamina Colombia — CasosMI Database
